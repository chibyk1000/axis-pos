  ...
  return (
    <Drawer open={open} onOpenChange={setOpen} direction="right">
      <DrawerContent className="w-full data-[vaul-drawer-direction=right]:sm:max-w-2xl bg-stone-50 dark:bg-stone-900 text-stone-900 dark:text-stone-100 border-l border-stone-200 dark:border-stone-700">
        <DrawerHeader className="flex flex-row items-center justify-between border-stone-200 dark:border-stone-700">
          {" "}
          <DrawerTitle className="text-xl font-semibold text-stone-900 dark:text-stone-100 flex ">
            {" "}
        ...
        <div className="space-y-4 p-4">
          <span className="text-sm text-stone-900 dark:text-stone-100">Name</span>
          <Input
            placeholder="Name"
            value={form.name}
            onChange={(e) => setForm({ ...form, name: e.target.value })}
          />
          <span className="text-sm text-stone-900 dark:text-stone-100">Position</span>
          <Input
          ...
          <span className="text-sm text-stone-900 dark:text-stone-100">Code</span>
          <Input
            placeholder="Code"
            value={form.code}
          ...
          <div className="flex flex-col gap-3">
            {booleanFields.map((field) => (
              <div key={field.key} className="flex items-center gap-x-3">
                <span className="text-sm text-stone-900 dark:text-stone-100">{field.label}</span>
                <Switch
